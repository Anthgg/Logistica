"""FastAPI router for Dispatch Document Preview, PDF, and Package Manifest endpoints (Phase 018).
...
def preview_dispatch_document(
    ...
    service = DispatchRenderingService(db)
    pdf_res = service.render_dispatch_preview(
    ...
    AuditService().record(
        ...
        resource_type="dispatch_document",
...
@router.post(
    ...
    summary="Evaluar y obtener el manifiesto del paquete documental de despacho",
)
def get_dispatch_package_manifest(
    payload: dict[str, Any],
    principal: LogisticsPrincipal = Depends(require_permission("logistics.documents.read")),
    db: Session = Depends(get_db),
) -> OutboundDispatchDocumentPackageManifest:
    ...
    AuditService().record(
        ...
        user_id=principal.user_id,
        session_id=principal.session_id,
        resource_type="dispatch_package_manifest",
