...
class ReceptionDifferenceApprovalService:
    ...
    def _emit(self, case: ReceptionDifferenceCaseModel, principal: LogisticsPrincipal, event_code: str, *, metadata: dict | None = None) -> None:
        ...
        self.db.add(ArrivalNoticeOutboxEventModel(
            ...
            aggregate_type="RECEPTION_DIFFERENCE_APPROVAL",
            ...
            deduplication_key=f"phase040:approval:{case.id}:{event_code}:{event_id}",
            status="PENDING",
        ))
        AuditService().write_event(self.db, AuditEventCommand(
            ...
            resource_type="reception_difference_approval",
    ...
    def create_approval_decision(
        ...
        approval_level = (self.db.scalar(
            select(func.max(ReceptionDifferenceApprovalModel.approval_level))
            .where(ReceptionDifferenceApprovalModel.difference_case_id == case_id)
        ) or 0) + 1

        ApprovalDecision(decision)

        approval = ReceptionDifferenceApprovalModel(
            id=uuid4(),
            difference_case_id=case_id,
            approval_level=approval_level,
            approver_user_id=principal.user_id,
            approver_snapshot=actor(principal),
            decision=decision,
            reason=reason,
            decided_at=now(),
        )
        ...
        self._emit(case, principal, "logistics.reception_difference.approval_decision_created", metadata={"decision": decision, "approval_level": approval_level})
        return approval
