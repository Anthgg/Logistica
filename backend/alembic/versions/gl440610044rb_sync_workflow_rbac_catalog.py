"""Synchronize implemented workflow permissions with the RBAC tables.

Revision ID: gl440610044rb
Revises: gk440510044rb
...
revision: str = "gl440610044rb"
down_revision: str | Sequence[str] | None = "gk440510044rb"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

_MIGRATION_NAMESPACE = UUID("d1e547cc-a5bc-4536-ae1c-5b11d98bdd41")
...
def _workflow_permissions() -> list[dict[str, object]]:
...
def upgrade() -> None:
    ...
    workflow_codes = [str(permission["code"]) for permission in workflow_permissions]
    ...
    permission_ids = dict(
        bind.execute(
            sa.select(permissions.c.code, permissions.c.id).where(
                permissions.c.code.in_(workflow_codes)
            )
        ).all()
    )
    role_ids = dict(bind.execute(sa.select(roles.c.code, roles.c.id)).all())

    # This repair intentionally grants only LOGISTICS_ADMIN. Expanding operational
    # roles requires a separate authorization review per organization.
    for role_code in ("LOGISTICS_ADMIN",):
        configured_codes = ROLE_PERMISSION_MATRIX[role_code]
...
def downgrade() -> None:
    ...
    workflow_codes = {str(permission["code"]) for permission in _workflow_permissions()}
    mapping_ids = [
        _stable_id("role-permission", f"LOGISTICS_ADMIN:{permission_code}")
        for permission_code in ROLE_PERMISSION_MATRIX["LOGISTICS_ADMIN"]
        if permission_code in workflow_codes
    ]
    permission_ids = [
        _stable_id("permission", permission_code)
        for permission_code in workflow_codes
    ]
