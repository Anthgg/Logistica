"""Synchronize the missing cost-center RBAC catalog entries.

Revision ID: gk440510044rb
Revises: gj440410044rb
Create Date: 2026-08-11 02:35:00.000000
"""
...
revision: str = "gk440510044rb"
down_revision: str | Sequence[str] | None = "gj440410044rb"
...
def _permissions() -> list[dict[str, object]]:
    from app.modules.logistics.rbac.permission_catalog import COST_CENTER_PERMISSIONS
...
def upgrade() -> None:
    ...
    configured_permissions = _permissions()
    codes = [str(permission["code"]) for permission in configured_permissions]
    ...
    permission_ids = dict(
        bind.execute(
            sa.select(permissions.c.code, permissions.c.id).where(permissions.c.code.in_(codes))
        ).all()
    )
...
def downgrade() -> None:
    ...
    codes = {str(permission["code"]) for permission in _permissions()}
    mapping_ids = [
    ...
    permission_ids = [_stable_id("permission", code) for code in codes]
