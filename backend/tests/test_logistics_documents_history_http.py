"""Regresión HTTP del historial de documentos (Fase 020).
...
`AttributeError` salía como 500 fuera del middleware CORS y el navegador lo
...
Estos van por HTTP a propósito, y NO mockean el servicio: el fallo estaba en la
comprobación de tenant del router, no en la consulta.
"""
...
def document_context(db_session: Session):
    """Organización, sede, tipo documental, usuario y borrador reales."""
    org = Organization(
        id=uuid4(),
...
def test_history_returns_events_for_authorized_organization(
    ...
    """El caso que reventaba en UAT: documento de la organización del principal."""
    ...
    assert entry["reason"] == "Emisión inicial"
...
def test_history_without_events_returns_empty_list(
    ...
    """Un documento sin eventos responde 200 con lista vacía, no 500."""
    document = document_context["document"]
...
def test_history_tolerates_missing_actor(
    ...
    document = document_context["document"]
    _add_event(
        db_session, document.id, actor_user_id=None, actor_snapshot=None
    )
    _add_event(
        db_session,
        document.id,
    ...
    assert names == [None, "Usuario Logístico"]
...
def test_history_response_carries_cors_headers(
    ...
    """La respuesta debe llegar al navegador: sin cabecera CORS parece un fallo de CORS."""
    document = document_context["document"]
    principal = _make_principal(
        [document_context["org"].id], document_context["actor"].id
    )
