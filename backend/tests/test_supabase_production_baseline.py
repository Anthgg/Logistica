...
def _get_restore_script_path() -> str:
    candidates = [
        os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "scripts", "restore_supabase_backup.py")),
        os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "scripts", "restore_supabase_backup.py")),
        os.path.abspath(os.path.join(os.getcwd(), "scripts", "restore_supabase_backup.py")),
    ]
    for c in candidates:
        if os.path.exists(c):
            return c
    pytest.skip("restore_supabase_backup.py not present in container environment")


def test_alembic_single_head():
    """Verify that Alembic has exactly one canonical HEAD and it is ik470110047dk."""
...
def test_restore_script_missing_force_flag():
    ...
    script_path = _get_restore_script_path()
